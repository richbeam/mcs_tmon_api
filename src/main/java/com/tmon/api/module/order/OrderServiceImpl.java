...
public class OrderServiceImpl implements OrderService{
	...
	public void delayOrders(Map<String, Object> order) throws UserDefinedException {
		...
		//주문순번(상세번호)
		orderMappingMap.put("detail_no", order.get("detail_no"));
		List<Map<String, Object>> orderList = basicSqlSessionTemplate.selectList("OrderMapper.selectTmonMappingInfo", orderMappingMap);
		...
		logMap.put("productno", order.get("productno"));
		logMap.put("sitename", "TMON");
		logMap.put("api_url", "/api/pd/1/listShppingDelay.ssg");		
		
		Map<String, Object> confirmParamMap = new HashMap<>();
		for(Map<String, Object> orderMap : orderList) {
			confirmParamMap.put("shppNo", orderMap.get("shppno"));
			confirmParamMap.put("shppSeq", orderMap.get("shppseq"));
			confirmParamMap.put("shortgRsnCd", "01");
			String whinExpcDt = HolidayUtil.workingDay(order.get("shpprsvtdt").toString(), 10);
			confirmParamMap.put("whinExpcDt", whinExpcDt);
			confirmParamMap.put("memoCntt", null);
			confirmParamMap.put("itemId", order.get("productno"));
			Map<String, Object> result = connector.listShppingDelay(confirmParamMap);
			
			//API 호출실패
			if(result == null || result.get("resultCode") == null) {
				logger.error(">>> delayOrders API fail : ordercd {} / result : {} /  confirmParamMap :  {}", order.get("ordercd"), result , confirmParamMap);
				logMap.put("content", "TMON API 호출 실패");
				logMap.put("status", order.get("status"));
				logUtil.insertOrderScheduleFailLog(logMap);
				return;
			}
			...
			//결과 저장
			logMap.put("content", result.get("resultDesc"));
			logMap.put("status", order.get("status"));
			if("00".equals(result.get("resultCode").toString())) {
				//정상호출건
				logger.error(">>> delayOrders success: {}", order.get("ordercd") + " confirmParamMap :" +  confirmParamMap + " result : " + result);
				logUtil.insertOrderScheduleSuccessLog(logMap);
			} else {
				//비정상 결과
				logger.error(">>> delayOrders fail: {}", order.get("ordercd") + ":confirmParamMap :" +  confirmParamMap + " result : " + result);
				//logUtil.insertOrderScheduleFailLog(logMap);
			}
		}
	}
	...
	public void compShipOrders(Map<String, Object> order) throws UserDefinedException {
		...
		//주문조회
		Map<String, Object> paramMap = new HashMap<>();
		paramMap.put("commType","01");
		paramMap.put("commValue",order.get("ordercd"));
		List<Map<String, Object>> result = connector.listNonDelivery(paramMap);
		
		if(result == null) {
			logger.info("발송대상주문 조회안됨[기처리건] : ordercd[" + order.get("ordercd") + "]");
			//처리결과
			tranDetail.put("apiindexing", "Y");
			basicSqlSessionTemplate.update("OrderMapper.updateCommOrderDetail", tranDetail); //comm_order_detail 등록
			//로그생성
			logMap.put("content", "발송대상주문 조회안됨[기처리건] : ordercd[" + order.get("ordercd") + "]");
			logMap.put("status", order.get("status"));
			logUtil.insertOrderScheduleFailLog(logMap);
		...
		//배송완료 처리
		Map<String, Object> deliveryEndParam;
		for(Map<String, Object> orderMap : result) {
			//운송장 등록 파라미터 생성
			deliveryEndParam = new HashMap<>();
			deliveryEndParam.put("shppNo",orderMap.get("shppNo"));
			deliveryEndParam.put("shppSeq",orderMap.get("shppSeq"));
			//출고처리 API 호출
			Map<String, Object> deliveryEndResult = connector.saveDeliveryEnd(deliveryEndParam);
			
			//API 호출실패
			logMap.put("api_url", "/api/pd/1/saveDeliveryEnd.ssg");
			if(deliveryEndResult == null || deliveryEndResult.get("resultCode") == null) {
				logger.error(">>> 배송완료 API 호출실패: {}", order.get("ordercd") );
				//처리결과
				tranDetail.put("status", BaseConst.OrderStauts.STAUTS_77);
				tranDetail.put("apiindexing", "N");
				basicSqlSessionTemplate.update("OrderMapper.updateCommOrderDetail", tranDetail); //comm_order_detail 등록
				...
				//로그생성
				logMap.put("content", "[배송완료] TMON API 호출 실패");
				logMap.put("status", order.get("status"));
				logUtil.insertOrderScheduleFailLog(logMap);
				continue;
			}
			
			if("00".equals(deliveryEndResult.get("resultCode").toString())) {
				//처리결과
				tranDetail.put("apiindexing", "Y"); 
				basicSqlSessionTemplate.update("OrderMapper.updateCommOrderDetail", tranDetail); //comm_order_detail 등록
				
				//로그 생성
				logMap.put("content", "[배송완료 처리완료]"+ deliveryEndResult.get("resultDesc"));
				logMap.put("status", order.get("status"));
				logUtil.insertOrderScheduleSuccessLog(logMap);
			} else if(deliveryEndResult.get("resultDesc") != null && deliveryEndResult.get("resultDesc").toString().contains("출고일") && deliveryEndResult.get("resultDesc").toString().contains("익일부터")) {
				//처리결과
				tranDetail.put("apiindexing", "Y"); 
				basicSqlSessionTemplate.update("OrderMapper.updateCommOrderDetail", tranDetail); //comm_order_detail 등록
				
				//로그 생성
				logMap.put("content", "[배송부분완료]"+ deliveryEndResult.get("resultDesc"));
				logMap.put("status", order.get("status"));
				logUtil.insertOrderScheduleSuccessLog(logMap);
			} else { 
				logger.error(">>> 주문확인 연계 실패: {}", order.get("ordercd"));
				...
				//비정상 결과
				logMap.put("content", "[배송완료 처리실패]"+ deliveryEndResult.get("resultDesc"));
				logMap.put("status", order.get("status"));
				logUtil.insertOrderScheduleFailLog(logMap);
			} 
		}
	...
	public void compShipOrdersByTmon(Map<String, Object> order) {
		...
		//comm_order_details 조회 
		Map<String, Object> commOrderDetail = basicSqlSessionTemplate.selectOne("OrderMapper.selectCommOrderDetailUpdate", paramMap);
		if (commOrderDetail == null) {
			logger.error(">>> 배송완료 대상없음: {}", order.get("orordNo"));
			return;
		}
		
		if (commOrderDetail.get("status").equals(OrderStauts.STAUTS_05)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_06)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_07)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_12)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_16)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_17)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_18)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_19)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_20)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_21)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_22)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_23)
				|| commOrderDetail.get("status").equals(OrderStauts.STAUTS_61)
		){
			logger.error(">>> 배송완료 대상없음: {}", order.get("ordNo"));
			return;
		}
		
		//주문로그
		Map<String, Object> logMap = new HashMap<String, Object>();
		logMap.put("ordercd" , order.get("orordNo"));
		logMap.put("m_ordercd" , order.get("orordItemSeq"));
		logMap.put("detail_no", commOrderDetail.get("detail_no"));
		logMap.put("productcd", commOrderDetail.get("productcd"));
		logMap.put("productno", commOrderDetail.get("productno"));
		logMap.put("sitename", "TMON");		
		logMap.put("status", commOrderDetail.get("status"));
		
		// 맵핑데이터 조회
		Map<String, Object> orderMappingMap = new HashMap<>();
		//주문코드
		orderMappingMap.put("ordercd", order.get("orordNo"));
		//주문순번(상세번호)
		orderMappingMap.put("detail_no", order.get("orordItemSeq"));
		List<Map<String, Object>> orderList = basicSqlSessionTemplate.selectList("OrderMapper.selectTmonMappingInfo", orderMappingMap);
		if (orderList == null) {
			logger.error(">>> 배송완료 대상없음: {}", order.get("orordNo"));
			return;
		...
		String shipfinisheddate = null;
		if (order.get("shppDt") != null ) {
			shipfinisheddate =  order.get("shppDt").toString();
		}
		//배송완료일
		paramMap.put("shipfinisheddate", shipfinisheddate);
		
		//배송완료처리
		int cnt = 0;
		if("15".equals(order.get("orordItemSeq").toString().trim())) {
			//교환배송인경우, 클레임아이가 최종클레임 아이디가 맞는지 확인하고 배송완료처리
			for(Map<String, Object> orderMap : orderList) {
				if(orderMap.get("claimshppno") == null) continue;
				if(!orderMap.get("claimshppno").toString().trim().equals(order.get("shppNo").toString().trim()) ) continue;
				
				//일반배송완료
				paramMap.put("status", OrderStauts.STAUTS_12);
				paramMap.put("apiindexing", "N");
				cnt = basicSqlSessionTemplate.update("OrderMapper.updateShipDate", paramMap);
			}
			
		} else if("11".equals(order.get("orordItemSeq").toString().trim())) {
			//교환배송등이 진행되지 않은경우 배송완료처리
			for(Map<String, Object> orderMap : orderList) {
				if(orderMap.get("claimshppno") != null) continue;
				
				//일반배송완료
				paramMap.put("status", OrderStauts.STAUTS_05);
				paramMap.put("apiindexing", "N");
				cnt = basicSqlSessionTemplate.update("OrderMapper.updateShipDate", paramMap);
			}
		}
		...
		if(cnt>0) {
			//로그생성
			logMap.put("content", "TMON 배송완료요청 처리");
			logUtil.insertOrderScheduleSuccessLog(logMap);
		}
	...
	public void shipChangeOrders(Map<String, Object> order) throws UserDefinedException {
		...
		if(orderList == null) {
			...
			logMap.put("content", "[교환발송처리 실패] 발송대상건 조회 실패" );
			logMap.put("status", order.get("status"));
			logUtil.insertOrderScheduleFailLog(logMap);
			return;
		}
		//================ 교환회수 프로세스 미처리될것 처리 Start ==================================
		Map<String, Object> paramMap = new HashMap<>();
		for(Map<String, Object> orderMap : orderList) {
			paramMap.put("commType","02");
			paramMap.put("commValue",orderMap.get("ordercd"));
			...
			for(Map<String, Object> resultMap : result) {
				...
				if("61".equals(lastShppProgStatDtlCd)) {
					...
					if(saveConfirmRcovResult == null || saveConfirmRcovResult.get("resultCode") == null) {
						logger.error(">>> [교환발송처리] 회수확인 API 호출실패: {}", order.get("ordercd") );
						logMap.put("content", "[교환발송처리 실패] 회수확인 API 호출실패");
						logMap.put("status", order.get("status"));
						logUtil.insertOrderScheduleFailLog(logMap);
						continue;
					}
					if("00".equals(saveConfirmRcovResult.get("resultCode").toString())) {
						//63:회수확인 상태로 후속프로세스 진행
						lastShppProgStatDtlCd = "63";
						//로그 생성
						logMap.put("content", "[교환발송처리-회수확인 완료]"+ saveConfirmRcovResult.get("resultDesc"));
						logMap.put("status", order.get("status"));
						logUtil.insertOrderScheduleSuccessLog(logMap); 
					} else {
						logger.error(">>> 교환발송처리-회수확인 연계 실패: {}", order.get("ordercd"));
						//처리결과
						tranDetail.put("status", BaseConst.OrderStauts.STAUTS_77);
						tranDetail.put("apiindexing", "N");
						basicSqlSessionTemplate.update("OrderMapper.updateCommOrderDetail", tranDetail); //comm_order_detail 등록
						...
						//비정상 결과
						logMap.put("content", "[교환발송처리-회수확인 처리실패]"+ saveConfirmRcovResult.get("resultDesc"));
						logMap.put("status", order.get("status"));
						logUtil.insertOrderScheduleFailLog(logMap);
					}
				}
				...
				if("63".equals(lastShppProgStatDtlCd)) {
					...
					if(saveCompleteRcovResult == null || saveCompleteRcovResult.get("resultCode") == null) {
						logger.error(">>> [교환발송처리] 회수완료 API 호출실패: {}", order.get("ordercd") );
						logMap.put("content", "[교환발송처리 실패] 회수완료 API 호출실패");
						logMap.put("status", order.get("status"));
						logUtil.insertOrderScheduleFailLog(logMap);
						continue;
					}
					if("00".equals(saveCompleteRcovResult.get("resultCode").toString())) {
						//63:회수확인 상태로 후속프로세스 진행
						//로그 생성
						logMap.put("content", "[교환발송처리-회수완료 완료]"+ saveCompleteRcovResult.get("resultDesc"));
						logMap.put("status", order.get("status"));
						logUtil.insertOrderScheduleSuccessLog(logMap);
					} else {
						logger.error(">>> 교환발송처리-회수완료 연계 실패: {}", order.get("ordercd"));
						//처리결과
						tranDetail.put("status", BaseConst.OrderStauts.STAUTS_77);
						tranDetail.put("apiindexing", "N");
						basicSqlSessionTemplate.update("OrderMapper.updateCommOrderDetail", tranDetail); //comm_order_detail 등록
						...
						//비정상 결과
						logMap.put("content", "[교환발송처리-회수완료 처리실패]"+ saveCompleteRcovResult.get("resultDesc"));
						logMap.put("status", order.get("status"));
						logUtil.insertOrderScheduleFailLog(logMap);
					}
				}
		...
		paramMap.put("commType","01");
		paramMap.put("commValue",order.get("ordercd"));
		List<Map<String, Object>> result = connector.listShppDirection(paramMap);
		
		//배송지시 발생했는지 체크
		if(result != null) {
			Map<String, Object> confirmParamMap = new HashMap<>();
			for(Map<String, Object> orderMap : result) {
				//배송상태코드가 정상인것만 주문확인처리
				if(!"10".equals(orderMap.get("shppStatCd").toString())) continue;
				
				confirmParamMap.put("shppNo", orderMap.get("shppNo"));
				confirmParamMap.put("shppSeq", orderMap.get("shppSeq"));
				Map<String, Object> confirmResult = connector.updateOrderSubjectManage(confirmParamMap);
				
				//API 호출실패
				logMap.put("api_url", "/api/pd/1/updateOrderSubjectManage.ssg");
				if(confirmResult == null || confirmResult.get("resultCode") == null) {
					logger.error(">>> [교환발송처리] 주문확인 API 호출실패: {}", order.get("ordercd") );
					logMap.put("content", "[교환발송처리 실패] 주문확인 API 호출실패");
					logMap.put("status", order.get("status"));
					logUtil.insertOrderScheduleFailLog(logMap);
					continue;
				}
				
				if("00".equals(confirmResult.get("resultCode").toString())) {
					//로그 생성
					logMap.put("content", "[교환발송처리-주문확인 완료]"+ confirmResult.get("resultDesc"));
					logMap.put("status", order.get("status"));
					logUtil.insertOrderScheduleSuccessLog(logMap);
				} else {
					logger.error(">>> 교환발송처리-주문확인 연계 실패: {}", order.get("ordercd"));
					//처리결과
					tranDetail.put("status", BaseConst.OrderStauts.STAUTS_77);
					tranDetail.put("apiindexing", "N");
					basicSqlSessionTemplate.update("OrderMapper.updateCommOrderDetail", tranDetail); //comm_order_detail 등록
					...
					//비정상 결과
					logMap.put("content", "[교환발송처리-주문확인 처리실패]"+ confirmResult.get("resultDesc"));
					logMap.put("status", order.get("status"));
					logUtil.insertOrderScheduleFailLog(logMap);
				}
			}
		...
		result = connector.listWarehouseOut(paramMap);
		if(result != null) {
			logMap.put("api_url", "/api/pd/1/saveWblNo.ssg");
			//택배회사 코드 조회
			Map<String, Object> deliveryCompany = null;
			if(order.get("delicomcd") != null) {
				paramMap.put("m_deliverycompanycode", order.get("delicomcd"));
				deliveryCompany = basicSqlSessionTemplate.selectOne("OrderMapper.selectTmonDelivery", paramMap);	
			}
			
			Map<String, Object> saveWblParamMap = null;
			Map<String, Object> saveWhOutCompleteProcessParamMap = null;
			for(Map<String, Object> orderMap : result) {
				//운송장 등록 파라미터 생성
				saveWblParamMap = new HashMap<>();
				saveWblParamMap.put("shppNo",orderMap.get("shppNo"));
				saveWblParamMap.put("shppSeq",orderMap.get("shppSeq"));
				//택배
				if("01".equals(order.get("shippingmethod"))) {
					if(null == order.get("shippingno")) {
						logger.error(">>> 송장번호없음 : {}", order);
						...
						//비정상 결과
						logMap.put("content", "[교환발송처리-송장번호없음]");
						logMap.put("status", order.get("status"));
						logUtil.insertOrderScheduleFailLog(logMap);
						
						continue;
					}
					if(null == order.get("delicomcd")) {
						logger.error(">>> 택배사번호없음 : {}", order);
						...
						//비정상 결과
						logMap.put("content", "[교환발송처리-택배사번호없음]");
						logMap.put("status", order.get("status"));
						logUtil.insertOrderScheduleFailLog(logMap);
					...
					if(deliveryCompany == null) {
						logger.error(" 택배회사 조회안됨 : ordercd[" + order.get("ordercd") + "]");
						...
						//비정상 결과
						logMap.put("content", "[교환발송처리-택배회사 없음]");
						logMap.put("status", order.get("status"));
						logUtil.insertOrderScheduleFailLog(logMap);
						
						continue;
					}
					saveWblParamMap.put("wblNo",order.get("shippingno"));
					saveWblParamMap.put("delicoVenId", deliveryCompany.get("deliverycompanycode"));
					saveWblParamMap.put("shppTypeCd","20");
					saveWblParamMap.put("shppTypeDtlCd","22");
					//택배사매핑가져오기
				//직배송	
				} else {
					saveWblParamMap.put("shppTypeCd","10");
					saveWblParamMap.put("shppTypeDtlCd","14");
				}
				//운송장 등록 API 호출
				Map<String, Object> saveWblResultMap = connector.saveWblNo(saveWblParamMap);
				if(saveWblResultMap == null || saveWblResultMap.get("resultCode") == null) {
					logger.error(">>> [교환발송처리-운송장등록] TMON API 호출 실패 : {}", saveWblParamMap);
					//처리결과
					tranDetail.put("status", BaseConst.OrderStauts.STAUTS_77);
					tranDetail.put("apiindexing", "N");
					basicSqlSessionTemplate.update("OrderMapper.updateCommOrderDetail", tranDetail); //comm_order_detail 등록
					...
					//로그생성
					logMap.put("content", "[교환발송처리-운송장등록] TMON API 호출 실패");
					logMap.put("status", order.get("status"));
					logUtil.insertOrderScheduleFailLog(logMap);
					continue;
				}
				...
				//API-호출결과
				if("00".equals(saveWblResultMap.get("resultCode").toString())) {				
					//로그 생성
					logMap.put("content", "[교환발송처리-운송장등록]"+ saveWblResultMap.get("resultDesc"));
					logMap.put("status", order.get("status"));
					logUtil.insertOrderScheduleSuccessLog(logMap);
				} else {
					logger.error(">>> 교환발송처리-운송장등록 실패: {}", order.get("ordercd"));
					...
					//비정상 결과
					logMap.put("content", "[교환발송처리-운송장등록실패]"+ saveWblResultMap.get("resultDesc"));
					logMap.put("status", order.get("status"));
					logUtil.insertOrderScheduleFailLog(logMap);
					continue;
				}
				...
				//출고처리 파라미터 생성
				saveWhOutCompleteProcessParamMap = new HashMap<>();
				saveWhOutCompleteProcessParamMap.put("shppNo",orderMap.get("shppNo"));
				saveWhOutCompleteProcessParamMap.put("shppSeq",orderMap.get("shppSeq"));
				saveWhOutCompleteProcessParamMap.put("procItemQty",order.get("qty"));
				//출고처리 API 호출
				Map<String, Object> saveResult = connector.saveWhOutCompleteProcess(saveWhOutCompleteProcessParamMap);
				
				logMap.put("api_url", "/api/pd/1/saveWhOutCompleteProcess.ssg");
				//API 호출실패
				if(saveResult == null || saveResult.get("resultCode") == null) {
					logger.error(">>> [교환발송처리] API 호출실패: {}", order.get("ordercd") );
					//처리결과
					tranDetail.put("status", BaseConst.OrderStauts.STAUTS_77);
					tranDetail.put("apiindexing", "N");
					basicSqlSessionTemplate.update("OrderMapper.updateCommOrderDetail", tranDetail); //comm_order_detail 등록
					...
					//로그생성
					logMap.put("content", "[교환발송처리] TMON API 호출 실패");
					logMap.put("status", order.get("status"));
					logUtil.insertOrderScheduleFailLog(logMap);
					continue;
				}
				...
				//API 호출결과 처리
				if("00".equals(saveResult.get("resultCode").toString())) {
					//처리결과
					tranDetail.put("apiindexing", "Y"); 
					basicSqlSessionTemplate.update("OrderMapper.updateCommOrderDetail", tranDetail); //comm_order_detail 등록
					
					//로그 생성
					logMap.put("content", "[교환발송처리완료]"+ saveResult.get("resultDesc"));
					logMap.put("status", order.get("status"));
					logUtil.insertOrderScheduleSuccessLog(logMap);
				} else {
					logger.error(">>> 발송처리 연계 실패: {}", order.get("ordercd"));
					...
					//비정상 결과
					logMap.put("content", "[교환발송처리 실패]"+ saveResult.get("resultDesc"));
					logMap.put("status", order.get("status"));
					logUtil.insertOrderScheduleFailLog(logMap);
				} 
			}
