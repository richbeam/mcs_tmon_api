...
public class ProdServiceImpl implements ProdService {
	...
	public Map<String, Object> getProducts(Map<String, Object> mProduct, Map<String, Object> tmonProduct, Map<String, Object> seller, List<Map<String, Object>> optionGroup,List<Map<String, Object>> optionList, String tType) {
		...
		try {
            ...
            if(tType.equals("I")){
                ...
                //배송지 셋팅 벤더사 정책번호(vendorPolicyNo) : 923
                product.put("vendorPolicyNo",923);                 	//  Integer+    티몬 오퍼레이터를 통해 받은 정책번호    O
	...
	public Map<String, Object> updateProductChange(Map<String, Object> change) {
		Map<String, Object> mhItem = new HashMap<>();
		mhItem.put("productcd", change.get("productcd"));
		
		Map<String, Object> result = updateProducts(mhItem);
		
		//결과저장
		Map<String, Object> sqlMap = new HashMap<>();
		sqlMap.put("productcd", change.get("productcd"));		
		if("00".equals(result.get("resultCode"))){
			sqlMap.put("ssg", "Y");
		} else {
			sqlMap.put("ssg", "E");
		}
		String gubun = change.get("gubun").toString();
		if(gubun.indexOf("price") > -1) {
			basicSqlSessionTemplate.update("ProdMapper.updateProductPriceChange", sqlMap);
		}
		if(gubun.indexOf("qty") > -1) {
			basicSqlSessionTemplate.update("ProdMapper.updateProductQtyChange", sqlMap);
		}
		if(gubun.indexOf("status") > -1) {
			basicSqlSessionTemplate.update("ProdMapper.updateProductStatusChange", sqlMap);
		}
	...
	public Map<String, Object> updateModifiedSellerProduct(Map<String, Object> change) {
		...
		//결과저장		
		if(result != null && result.get("status") != null && !"03".equals(result.get("status"))){
			String sellerSsg =  change.get("sellerssg").toString();
			if("D".equals(sellerSsg) || "R".equals(sellerSsg)) {
				Map<String, Object> sqlMap = new HashMap<>();
				sqlMap.put("productcd", change.get("productcd"));
				sqlMap.put("ssg", sellerSsg);
				basicSqlSessionTemplate.update("ProdMapper.updateSsgProducts", sqlMap);
			}
		}
	...
	public Map<String, Object> updateDeletedSellerProduct(Map<String, Object> change) {
		...
		Map<String, Object> mhItem = new HashMap<>();
		mhItem.put("productcd", change.get("productcd"));
		mhItem.put("status", "03");
		Map<String, Object> result = updateProducts(mhItem);
		
		//결과저장		
		if(result != null && result.get("status") != null && !"03".equals(result.get("status"))){
			String sellerSsg =  change.get("sellerssg").toString();
			if("D".equals(sellerSsg)) {
				Map<String, Object> sqlMap = new HashMap<>();
				sqlMap.put("productcd", change.get("productcd"));
				sqlMap.put("ssg", sellerSsg);
				basicSqlSessionTemplate.update("ProdMapper.updateSsgProducts", sqlMap);
	...
	public String setAddrId(Map<String, Object> seller, String gubuncd) throws UserDefinedException {
		//파라미터 셋팅
		//별칭
		String addrlcAntnmNm = null;
		if(seller.get("businame") != null) {
			addrlcAntnmNm = seller.get("businame").toString().trim();
		}
				
		//우편번호
		String postCode = "";
		//기본 주소
		String addrBasc = null;
		//상세주소
		String addrDtl = null;
		//택배사 계약코드
		String delicoVenId = null;
				
		//반품주소 gubuncd : "01"
		//출고주소 gubuncd : "02"
		if("01".equals(gubuncd)) {
			// 반품지 제약사항 체크
			if(seller.get("returnpostcode1") == null && seller.get("returnpostcode2") == null) return null;
						
			if(seller.get("returnpostcode1") != null) {
				postCode = seller.get("returnpostcode1").toString();
			}
			if(seller.get("returnpostcode2") != null) {
				postCode += seller.get("returnpostcode2").toString();
			}
			if(seller.get("returnaddress1") != null) {
				addrBasc = seller.get("returnaddress1").toString().trim();
			}			
			if(seller.get("returnaddress2") != null) {
				addrDtl = seller.get("returnaddress2").toString().trim();
			}
			delicoVenId = "0000033028";
		} else if("02".equals(gubuncd)) {
			// 출고지 제약사항 체크
			if(seller.get("postcode1") == null && seller.get("postcode2") == null) return null;
			
			if(seller.get("postcode1") != null) {
				postCode = seller.get("postcode1").toString();
			}
			if(seller.get("postcode2") != null) {
				postCode += seller.get("postcode2").toString();
			}	
			if(seller.get("address1") != null) {
				addrBasc = seller.get("address1").toString().trim();
			}			
			if(seller.get("address2") != null) {
				addrDtl = seller.get("address2").toString().trim();
			}
		} else {
			throw new UserDefinedException(UserDefinedException.ETC_ERROR, "출고/반품 주소유형이 없습니다.");
		}
		
		//전화번호
		String cnts = null;
		if(seller.get("tel") != null) {
			cnts = seller.get("tel").toString().trim();
			String tel =seller.get("tel").toString().trim().split("-")[0];
			if(tel.contains("00") || tel.contains("000")) {
				cnts = seller.get("tel").toString().trim().split("-")[1]+"-"+seller.get("tel").toString().trim().split("-")[2];
				//cnts = seller.get("tel").toString().trim().replace("^00-", "");
			}
		}
		
		if (seller.get("tel").equals("1544-08-07")){
			cnts="1544-0807";
		}

		Map<String, Object> venAddr = new HashMap<>();
		venAddr.put("addrlcAntnmNm", addrlcAntnmNm);
		venAddr.put("zipcd", postCode);
		venAddr.put("doroAddrBasc", addrBasc);
		venAddr.put("doroAddrDtl", addrDtl);
		venAddr.put("jibunAddrBasc", addrBasc);
		venAddr.put("jibunAddrDtl", addrDtl);
		venAddr.put("bascAddrYn", "Y");
		venAddr.put("cnts", cnts);
		venAddr.put("delicoVenId", delicoVenId);
		
		String doroAddrId = null;
		String jibunAddrId = null;
		//기등록건 조회
		Map<String, Object> paramMap = new HashMap<>();
		paramMap.put("sellercd", seller.get("sellercd").toString());
		paramMap.put("zipcd", postCode);
		paramMap.put("gubuncd", gubuncd);
		Map<String, Object> deliveryPolicy = basicSqlSessionTemplate.selectOne("ProdMapper.selectSsgDeliveryPolicy", paramMap);
		//logger.warn("deliveryPolicy : {} paramMap : {}" , deliveryPolicy, paramMap);
		// 배송정책이 없다면 신규등록, 있으면 수정
		if(deliveryPolicy == null) {
			Map<String, Object> venAddrResult = connector.insertVenAddr(venAddr);
			if(venAddrResult != null) {
				if(venAddrResult.get("doroAddrId") != null) {
					doroAddrId = venAddrResult.get("doroAddrId").toString();	
				}
				if(venAddrResult.get("jibunAddrId") != null) {
					jibunAddrId = venAddrResult.get("jibunAddrId").toString();	
				}
				deliveryPolicy = new HashMap<>();
				if(venAddrResult.get("grpAddrId") != null) {
					deliveryPolicy.put("grpaddrid", venAddrResult.get("grpAddrId").toString());	
				}
				deliveryPolicy.put("doroaddrid", doroAddrId);
				deliveryPolicy.put("jibunaddrid", jibunAddrId);
				deliveryPolicy.put("gubuncd", gubuncd);
				deliveryPolicy.put("sellercd", seller.get("sellercd").toString());
				deliveryPolicy.put("zipcd", postCode);
				deliveryPolicy.put("addrlcantnm", addrlcAntnmNm);
				deliveryPolicy.put("addrbasc", addrBasc);
				deliveryPolicy.put("addrdtl", addrDtl);
				deliveryPolicy.put("cnts", cnts);
				basicSqlSessionTemplate.insert("ProdMapper.insertSsgDeliveryPolicy", deliveryPolicy);
			}
		} else {
			//logger.warn("addrlcAntnmNm : {} addrlcAntnmNm : {}" , addrlcAntnmNm, deliveryPolicy.get("addrlcantnm").toString());
			//logger.warn("addrBasc : {} addrBasc : {}" , addrBasc, deliveryPolicy.get("addrbasc").toString());
			//logger.warn("addrDtl : {} addrDtl : {}" , addrDtl, deliveryPolicy.get("addrdtl").toString());
			//logger.warn("cnts : {} cnts : {}" , cnts, deliveryPolicy.get("cnts").toString());
			if(!addrlcAntnmNm.equals(deliveryPolicy.get("addrlcantnm").toString())
					|| !addrBasc.equals(deliveryPolicy.get("addrbasc").toString())
					|| !addrDtl.equals(deliveryPolicy.get("addrdtl").toString())
					|| !cnts.equals(deliveryPolicy.get("cnts").toString())){
				venAddr.put("grpAddrId", deliveryPolicy.get("grpaddrid"));
				Map<String, Object> venAddrResult = connector.updateVenAddr(venAddr);
				
				//logger.warn("venAddrResult : {} venAddr : {}" , venAddrResult, venAddr);
				...
				if(venAddrResult != null) {
					...
					deliveryPolicy.put("addrdtl", addrDtl);
					deliveryPolicy.put("cnts", cnts);
					basicSqlSessionTemplate.insert("ProdMapper.updateSsgDeliveryPolicy", deliveryPolicy);
				}
			} else {
				doroAddrId =deliveryPolicy.get("doroaddrid").toString();
				jibunAddrId = deliveryPolicy.get("jibunaddrid").toString();	
			}
		}

		//logger.warn("jibunAddrId : {} doroAddrId : {}" , jibunAddrId, doroAddrId);

		if(jibunAddrId == null) return doroAddrId;
		return jibunAddrId;
	}
	...
	public String setShppcstPlcy(String shppcstPlcyDivCd, String shppcstAplUnitCd, Map<String, Object> item) throws UserDefinedException {
		Map<String, Object> shippingPolicyParamMap = new HashMap<>();
		//배송비정책구분코드(10 출고배송비 정책, 20 반품배송비 정책, 60 추가배송비 정책(도서산간), 70 추가배송비 정책(제주))
		shippingPolicyParamMap.put("shppcstPlcyDivCd", shppcstPlcyDivCd);
		/*
		 * 배송비 적용 단위 코드 (commCd:I113)
		 * 출고, 추가 배송비 정책 등록 시 (shppcstPlcyDivCd = 10, 60, 70) :
		 *   10 주문 금액 합산
		 *   30 상품 수량 별
		 *   40 상품별 주문금액 합산 
		 * 반품 배송비 정책 등록 시  (shppcstPlcyDivCd = 20) :
		 *   10 주문 금액 합산
		 *   30 상품 수량 별
		 */
		shippingPolicyParamMap.put("shppcstAplUnitCd", shppcstAplUnitCd);
		
		//선불착불구분코드(10 배송비선불, 20 배송비착불)
		String prpayCodDivCd = "10";
		if ("03".equals(item.get("shippingfeepaytype"))) {  //멸치 - 01:무료,02:착불/선결제,03:착불만 가능,04:선결제만 가능
			 prpayCodDivCd = "20";
		}
		shippingPolicyParamMap.put("prpayCodDivCd", prpayCodDivCd);
		
		//배송비
		int shippingfee = 0;
		if("60".equals(shppcstPlcyDivCd) || "70".equals(shppcstPlcyDivCd)) {
			//추가배송비
			if(item.get("additionalshippingfee") != null) {
				shippingfee = Integer.parseInt(item.get("additionalshippingfee").toString());
			}
		} else if("20".equals(shppcstPlcyDivCd)) {
			//배송비정책구분코드(20 반품배송비 정책
			//반품배송비는 기본배송비 + 반품배송비 (반약 기본배송비가 없다면 반품배송비*2)
			String shippingfeetype = item.get("shippingfeetype") != null ? item.get("shippingfeetype").toString().trim() : null;
			if(item.get("shippingfee") != null && !"01".equals(shippingfeetype)) {
				shippingfee = Integer.parseInt(item.get("shippingfee").toString());
			}
			if(shippingfee > 0) {
				if(item.get("returnshippingfee") != null) {
					shippingfee = shippingfee + Integer.parseInt(item.get("returnshippingfee").toString());
				}	
			} else {
				shippingfee = Integer.parseInt(item.get("returnshippingfee").toString()) * 2;
			}
			
		} else {
			//배송료는 무료로 진행(상품판매금액에 녹임
			//if(item.get("shippingfee") != null) {
			//	shippingfee = Integer.parseInt(item.get("shippingfee").toString());
			//}	
			shippingfee = 0;
		}
		...
		shippingPolicyParamMap.put("shppcst", shippingfee);
		
		//기본 정책 여부(Y/N)
		shippingPolicyParamMap.put("bascPlcyYn", "Y");
		
		//배송비 적용 여부(Y/N)
		String shppcstAplYn = "Y";
//			 if (item.get("shippingfeetype") != null && "01".equals(item.get("shippingfeetype").toString())) {  //[멸치 - 01:무료 배송,02:유료 배송,03:유료배송(예외),04:유료배송(조건부) ]
//				 shppcstAplYn = "N";
//			 }
		shippingPolicyParamMap.put("shppcstAplYn", shppcstAplYn);
		
		//면제 기준 금액[출고배송비 정책 등록시에만 사용(shppcstPlcyDivCd = 10)]
		if("10".equals(shppcstPlcyDivCd )) {
			int shppcstExmpCritnAmt = 999999999;
			if(item.get("shippingfeetype") != null && "01".equals(item.get("shippingfeetype").toString())) { //[멸치 - 01:무료 배송,02:유료 배송,03:유료배송(예외),04:유료배송(조건부) ]
				shppcstExmpCritnAmt = 0;
			} else if(item.get("freeshippingamount") != null && Integer.parseInt(item.get("freeshippingamount").toString()) > 0) {
				shppcstExmpCritnAmt = Integer.parseInt(item.get("freeshippingamount").toString());
			}
			shippingPolicyParamMap.put("shppcstExmpCritnAmt", shppcstExmpCritnAmt);
		}
				
		//배송정책 테이블에서 등록된 배송정책이 있는지 조회 
		Map<String, Object> shippingPolicy = basicSqlSessionTemplate.selectOne("ProdMapper.selectSsgShippingPolicy", shippingPolicyParamMap);
		String whoutShppcstId = "";
		if (shippingPolicy != null) {
			whoutShppcstId = shippingPolicy.get("shppcstid").toString();
		} else { 
			whoutShppcstId = connector.addShppcstPlcy(shippingPolicyParamMap);
			shippingPolicyParamMap.put("shppcstId", whoutShppcstId);
			basicSqlSessionTemplate.selectOne("ProdMapper.insertSsgShippingPolicy", shippingPolicyParamMap);
		}
	...
	public Map<String, Object> getApprovalPengding(Map<String, Object> change){
		String itemId = change.get("productno").toString();
		String productCd = change.get("productcd").toString();
		Map<String, Object> response = null;
		Map<String, Object> result = new HashMap<>();
		
		//로그 기본값 셋팅
		Map<String, Object> logMap = new HashMap<>();
		logMap.put("productcd", productCd);
		logMap.put("api_url", "/item/0.1/getItemChngDemandList.ssg");
		logMap.put("status", "99");
		
		result.put("productcd", productCd);
		result.put("productno", itemId);
		result.put("url", "/item/0.1/getItemChngDemandList.ssg");
		result.put("sellingprice", change.get("sellingprice"));
		result.put("supplyprice", change.get("supplyprice"));
		
		try {
			response = connector.getApprovalPending(itemId);
			if(response.get("itemChngDemnd") != null) {
				List<Map<String, Object>> resultItemChngDemnd = (List<Map<String, Object>>) response.get("itemChngDemnd");
				for(Map<String, Object> chngDemnd : resultItemChngDemnd) {
					//맵핑테이블 업데이트
					String approvalStatus = chngDemnd.get("chngDemndProcStatCd").toString();
					String approvalMessege = chngDemnd.get("chngDemndProcStatNm").toString();
					
					if(approvalStatus != null) {
						if(!approvalStatus.equals("20")) {
							Map<String, Object> paramMap = new HashMap<>();
							paramMap.put("productcd", productCd);
							paramMap.put("ssg", "B");
							basicSqlSessionTemplate.update("ProdMapper.updateSsgProducts", paramMap);
							
							logMap.put("content", approvalMessege);
							logUtil.insertProdScheduleSuccessLog(logMap);
							
							result.put("status", "01");
							result.put("contents","상품 승인 목록 조회 성공 : "+approvalMessege);
						}
					}
			...
			if(response == null || response.get("resultCode") == null || !"00".equals(response.get("resultCode"))){
				logMap.put("contents", response.get("resultDesc"));	
				result.put("status", "01");
				result.put("contents","상품 승인 목록 조회 실패 : "+response.get("resultDesc"));
				logUtil.insertProdScheduleFailLog(logMap);
			}
			...
			result.put("contents",e.getMessage());
			logUtil.insertProdScheduleFailLog(logMap);
		}
		return result;
	}
	...
	//승인완료 조회
					...
					for(Map<String, Object> chngDemnd : resultItemChngDemnd) {
						...
						if(approvalStatus != null) {
							if(approvalStatus.equals("20")) {
								...
								logger.warn("승인완료 해당없음 : "+ approvalMessege);
							}
						}
		...
		public Map<String, Object> getProductStatus(Map<String, Object> request) {
			String itemId = request.get("productno").toString();
			String productCd = request.get("productcd").toString();
			Map<String, Object> response = null;
			Map<String, Object> result = new HashMap<>();
			Map<String, Object> sqlMap = new HashMap<>();
			...
			Map<String, Object> logMap = new HashMap<>();
			logMap.put("productcd", productCd);
			logMap.put("productno", itemId);
			logMap.put("api_url", "/item/0.3/viewItem.ssg");
			logMap.put("status", "98");
			
			result.put("productcd", productCd);
			result.put("productno", itemId);
			result.put("url", "/item/0.3/viewItem.ssg");
			result.put("sellingprice", request.get("sellingprice"));
			result.put("supplyprice", request.get("supplyprice"));
			
			try {
				response = connector.getItemStatus(itemId);
				if(response.get("sellStatCd") != null) {
					if(response.get("sellStatCd").equals("20")) {
						logMap.put("content","판매중지 동기화 처리");
						logUtil.insertProdScheduleSuccessLog(logMap);
						
						sqlMap.put("isAppr", "Y");
						basicSqlSessionTemplate.insert("ProdMapper.updateSsgProducts", sqlMap);
						
						//updateDeletedSellerProduct(request);
						updateProducts(request);
						
						result.put("status", "02");
						result.put("contents","상품 상세조회 성공 :(" +response.get("sellStatCd")+ ") 판매중");
					} else {
						logger.warn("판매중지 조회대상 제외 : {}",response);
						logMap.put("content","판매중지 조회대상 제외");
						logUtil.insertProdScheduleSuccessLog(logMap);
						
						result.put("status", "01");
						result.put("contents","상품 상세조회 성공 :(" +response.get("sellStatCd")+ ") 판매중지");
					}												
				}
				
				if(response == null || response.get("resultCode") == null || !"00".equals(response.get("resultCode"))){
					logMap.put("content", response.get("resultDesc"));	
					result.put("status", "03");
					result.put("contents","상품 상세조회 실패 : "+response.get("resultDesc"));
					logUtil.insertProdScheduleFailLog(logMap);
				}
