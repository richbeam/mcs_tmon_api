...
public class ClaimScheduler {
	...
	public void cancelOrders() throws Exception {
		...
		List<Map<String, Object>> orders = connector.inquiry(paramMap);

		logger.info(">>cancelOrders list : {} param : {} ", orders, paramMap);

		if(orders != null) {
			for(Map<String, Object> order : orders) {
				try {
					claimService.cancelOrders(order);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		} else {
			logger.info(">>> 주문조회 없음: {}", orders);
		}
		logger.warn(">>취소조회 취소요청 시작");
	}
	...
	//반품교환회수 (반품요청, 반품수거완료, 반품환불완료)
	//@Scheduled(cron = "0 0/12 * * * *")
	public void returnOrders() throws Exception {
		logger.warn(">>반품교환회수 요청처리 시작");

		//대상조회 - 접수된 주문 조회
		Map<String, Object> paramMap = new HashMap<>();
		paramMap.put("perdType", "01");   //기간타입 - 01 회수지시일
		String today = StringUtil.getTodayString("yyyyMMdd");
		paramMap.put("perdStrDts", StringUtil.dateAdd(today, -15));
		paramMap.put("perdEndDts", today);
		paramMap.put("commType", null);
		paramMap.put("commValue", null);
		List<Map<String, Object>> orders = connector.listExchangeTarget(paramMap);

		if(orders != null) {
			for(Map<String, Object> order : orders) {
				try {
					claimService.returnOrders(order);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	//반품 완료
	//@Scheduled(cron = "0 0/12 * * * *")
	public void completeReturnOrders() throws Exception {
		logger.info(">>반품회수완료 시작");
		//대상 조회
		Map<String, Object> paramMap = new HashMap<>();
		paramMap.put("sitename"    , "TMON");              //사이트명
		paramMap.put("status"      , OrderStauts.STAUTS_15); //주문상태 반품확인중
		paramMap.put("apiindexing" , "U");                   //멸치쇼핑 이관여부
		List<Map<String, Object>> orders = basicSqlSessionTemplate.selectList("ClaimMapper.selectCommOrderDetail", paramMap);
		...
		if(orders != null) {
			for(Map<String, Object> order : orders) {
				try {
					claimService.completeReturnOrders(order);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		...
		logger.info(">>반품회수완료 종료");
	}
	
	//교환 완료
	//@Scheduled(cron = "0 0/11 * * * *")
	public void completeExchangeOrders() throws Exception {
		logger.info(">>교환회수완료 시작");
		//대상 조회
		Map<String, Object> paramMap = new HashMap<>();
		paramMap.put("sitename"    , "TMON");              //사이트명
		paramMap.put("status"      , OrderStauts.STAUTS_10); //주문상태 교환회수완료
		paramMap.put("apiindexing" , "U");                   //멸치쇼핑 이관여부
		List<Map<String, Object>> orders = basicSqlSessionTemplate.selectList("ClaimMapper.selectCommOrderDetail", paramMap);
